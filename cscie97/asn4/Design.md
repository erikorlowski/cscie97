...
## Exception Handling
Exceptions are primarily handled by the four Exception classes. The InvalidAccessTokenException class handles cases where an AccessToken is expired or logged out. The AccessDeniedException handles cases where an access request is rejected because the AccessToken provided does not have the correct access. The AuthenticationException handles errors during user authentication.

These three classes extend the EntitlementException class, which is meant to be a catch-all for Entitlement Service related Exceptions. The EntitlementException class extends the RuntimeException class to ensure that Exceptions generated by the Housemate Entitlement Service are handled explicitly.
